...
<body>
			...
			<div class="content">
				...
				<form class="form-1" method="POST" action="app/update-task.php">
					...
					<div class="input-holder">
						<lable>Title</lable>
						<input type="text" name="title" class="input-1" placeholder="Task title" value="<?=$task['title']?>" required>
					</div>
					
					<div class="input-holder">
						<lable>Description</lable>
						<textarea name="description" rows="5" class="input-1"><?=$task['description']?></textarea>
					</div>
					...
					<div class="input-holder">
						...
						<?php 
                            $assignee_ids = get_task_assignee_ids($conn, $task['id']);
                            $current_assignee_id = 0;
                            $current_assignee_name = "";
                            if (!empty($assignee_ids)) {
                                $current_assignee_id = $assignee_ids[0]; // Take the first one as we are moving to single user
                                // Find name
                                foreach($users as $user){
                                    if($user['id'] == $current_assignee_id){
                                        $current_assignee_name = $user['full_name'];
                                        break;
                                    }
                                }
                            }
                        ...
                        <div class="assign-container">
                            ...
                            <div class="user-dropdown" id="userDropdown">
                                <?php if (!empty($users)) { 
                                    foreach ($users as $user) {
                                        $checked = ($user['id'] == $current_assignee_id) ? 'checked' : '';
                                ?>
                                <label class="dropdown-item" style="display: flex; align-items: center; justify-content: space-between;">
                                    ...
                                    <input type="radio" name="assigned_user_radio" class="user-radio" value="<?=$user['id']?>" data-name="<?=$user['full_name']?>" <?=$checked?> style="accent-color: var(--primary-color);">
                                </label>
                                <?php } } else { ?>
                        ...
                        <input type="hidden" name="assigned_to" id="assignedToInput" value="<?=$current_assignee_id != 0 ? $current_assignee_id : ''?>">
					</div>
    ...
    <script>
        ...
        pillsContainer.addEventListener('click', toggleDropdown);
